...
| Screenshots | `org.freedesktop.portal.Screenshot`, then an editor | `spectacle`, or `grim` plus `slurp`, or an error |
...
a region, cropped, with the irrelevant half redacted and an arrow at the
thing being asked about. The portal does not do any of that -- it hands
back a picture -- so Ctrl+Alt+G has three tiers and takes the first that
works:

| Tier | What runs | Where |
| --- | --- | --- |
| A desktop editor | `spectacle -r`, or `grim`+`slurp` piped to `satty` or `swappy` | wherever one is installed |
| dictatr's own | the portal grabs the screen, `ui/annotate.py` crops and marks it | anywhere with GTK |
| The bare portal | one capture, no editing | headless, or no GTK |

None of them is a dependency. Spectacle ships with Plasma and is better
than ours, so KDE keeps it; a desktop with nothing gets the same editor
as every other desktop with nothing, which is the point of having
written one.

The editor is drag-to-select, then `r` box, `a` arrow, `p` pen, `b`
redact, `c` back to selecting, `u` undo, Enter to save, Esc to cancel.
Enter without selecting saves the whole screen. Redaction is a mosaic
rather than a blur, because a blur of readable text is sometimes
reversible.

`screenshot = dictatr` uses ours everywhere, `portal` skips editing
entirely, and any other value is a command line with `{path}` in it
(`screenshot = flameshot gui -p {path}`).
...
| `DICTATE_SCREENSHOT` | `auto` | screenshot tool: `auto`, `dictatr`, `portal`, or a command with `{path}` |
